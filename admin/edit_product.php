<?php
  ...
  if (isset($_GET['id'])) {
    $query = query("SELECT * FROM product WHERE p_id =" . escape_string($_GET['id']) ."");
    confirm($query);

    while ($row = fetch_array($query)) {
        $product_title          = escape_string($row['p_name']);
        $product_category_id    = escape_string($row['p_cat']);
        $product_price          = escape_string($row['p_price']);
        $product_description    = escape_string($row['p_description']);
        $product_quantity       = escape_string($row['p_quantity']);
        $product_image = escape_string($row['p_image']);
    }
    update_product();
  }
?>
...
<!-- 
 ...
 -->
    ...
    <div class="form-group">
        ...
        <img width="200" src="../images/product/<?php echo $product_image; ?>">
    </div>
